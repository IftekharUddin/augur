...
// Augur fork: the inherited release workflow is parked under
// `.github/workflows-upstream/` so it cannot fire on a version tag with
// upstream's runners and publishing identity. It is still the canonical
// source of the prebuilt-binary target matrix, so the generator follows it.
// See docs/architecture/fork-touchpoints.md.
const RELEASE_WORKFLOW: &str = ".github/workflows-upstream/release-stable-manual.yml";
